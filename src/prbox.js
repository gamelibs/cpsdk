...
wop.startOrientationWatcher = function (intervalMs = 1000) {
    try {
        ...
        // Run an immediate check and then poll (polling kept for older environments)
        try {
            wop.checkOrientation();
        } catch (e) {}
        wop._orientIntervalId = setInterval(function () { wop.checkOrientation(); }, Number(intervalMs) || 1000);
    } catch (e) { }
}
...
!(function () {
    ...
    wop.checkOrientation = function () {
        ...
        // Robust detection: prefer Screen Orientation API, then matchMedia, then fallback to window.orientation / innerWidth/innerHeight
        let isLandscape = false;

        try {
            if (navigator.userAgent && /ipad|iphone|ipod/i.test(navigator.userAgent) && typeof window.screen !== 'undefined' && typeof window.screen.orientation === 'object' && typeof window.screen.orientation.type === 'string') {
                // iOS Safari may report orientation differently; check type
                isLandscape = window.screen.orientation.type.indexOf('landscape') !== -1;
                
            } else if (typeof screen !== 'undefined' && screen.orientation && typeof screen.orientation.type === 'string') {
                isLandscape = screen.orientation.type.indexOf('landscape') !== -1;
